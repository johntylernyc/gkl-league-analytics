#!/usr/bin/env python3
"""
Collect daily (game-by-game) player stats from Yahoo Fantasy API
INCLUDING calculated stats (AVG, OBP, SLG, OPS)
"""
...
# Complete Yahoo stat ID mappings for daily baseball stats
DAILY_STAT_ID_MAP = {
    # Batting counting stats
    '7': 'runs',          # R
    '8': 'hits',          # H  
    '10': 'doubles',      # 2B (if provided)
    '11': 'triples',      # 3B
    '12': 'home_runs',    # HR
    '13': 'rbis',         # RBI
    '16': 'stolen_bases', # SB
    '18': 'walks',        # BB (if provided)
    '21': 'strikeouts',   # K (batting)
    '60': 'at_bats_hits', # H/AB format (e.g., "4/6" means 4 hits in 6 at-bats)
    
    # Batting rate stats (calculated by Yahoo)
    '3': 'avg',           # AVG - Batting Average
    '4': 'obp',           # OBP - On-Base Percentage
    '5': 'slg',           # SLG - Slugging Percentage
...
class DailyStatsCollector:
                    ...
                    for stat in player_stats.findall('.//fantasy:stats/fantasy:stat', ns):
                        ...
                        if stat_id_elem is not None and value_elem is not None:
                            ...
                            if stat_id in DAILY_STAT_ID_MAP and value and value != '-':
                                ...
                                elif stat_name in ['avg', 'obp', 'slg']:
                                    try:
                                        # Convert .000 format to float
                                        player_data['batting_stats'][stat_name] = float(value)
                                    except ValueError:
                                        player_data['batting_stats'][stat_name] = 0.0
                                # Pitching stats
    ...
    def save_daily_stats(self, all_players_data, date_str):
        ...
        for player in all_players_data:
            ...
            try:
                ...
                # Use Yahoo's calculated AVG or calculate if not provided
                batting_avg = batting.get('avg', 0.0)
                if batting_avg == 0.0 and batting.get('at_bats', 0) > 0 and 'hits' in batting:
                    batting_avg = batting['hits'] / batting['at_bats']
                
                # Use Yahoo's calculated OBP and SLG
                batting_obp = batting.get('obp', 0.0)
                batting_slg = batting.get('slg', 0.0)
                
                # Calculate OPS
                batting_ops = batting_obp + batting_slg
                ...
                cursor.execute(f"""
                    INSERT OR REPLACE INTO {self.config['gkl_player_stats_table']} (
                        ...
                        batting_home_runs, batting_stolen_bases, 
    ...
    def show_daily_results(self, date_str):
        """Show sample of daily stats collected including rate stats"""
        cursor = self.conn.cursor()
        
        print(f"\n{'='*130}")
        print(f"DAILY STATS FOR {date_str} (with AVG, OBP, SLG, OPS)")
        print(f"{'='*130}")
        
        # Show batting stats
        print("\nBATTING PERFORMANCES:")
        print(f"{'Player':<25} {'Team':<5} {'AB':<4} {'H':<3} {'R':<3} {'RBI':<4} {'HR':<3} {'SB':<3} {'AVG':<6} {'OBP':<6} {'SLG':<6} {'OPS':<6}")
        print("-" * 130)
        ...
        cursor.execute(f"""
            SELECT 
                ...
                batting_home_runs, batting_stolen_bases,
                ROUND(batting_avg, 3), ROUND(batting_obp, 3), 
                ROUND(batting_slg, 3), ROUND(batting_ops, 3)
        ...
        for row in cursor.fetchall():
            ...
            sb = str(row[7]).rjust(3)
            avg = f"{row[8]:.3f}".ljust(6)
            obp = f"{row[9]:.3f}".ljust(6)
            slg = f"{row[10]:.3f}".ljust(6)
            ops = f"{row[11]:.3f}".ljust(6)
            
            print(f"{name} {team} {ab} {h} {r} {rbi} {hr} {sb} {avg} {obp} {slg} {ops}")
        ...
        if summary:
            print(f"\n\nDAILY SUMMARY:")
            print(f"Total players with at-bats: {summary[0]}")
            print(f"League averages - AVG: {summary[1]:.3f}, OBP: {summary[2]:.3f}, SLG: {summary[3]:.3f}, OPS: {summary[4]:.3f}")
    ...
    def run_collection(self, date_str=None, days_back=1):
        ...
        print(f"\nRate stats (AVG, OBP, SLG, OPS) are now captured!")
        print("Note: These are Yahoo's calculated daily values")
        print("For period aggregation, we'll need to calculate weighted averages")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Collect daily stats from Yahoo API with rate stats")
