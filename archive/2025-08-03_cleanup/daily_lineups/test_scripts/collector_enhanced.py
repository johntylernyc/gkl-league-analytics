...
class EnhancedLineupsCollector(DailyLineupsCollector):
    ...
    def validate_collection_date(self, date_str: str) -> bool:
        """
        Validate that a date is not in the future and is appropriate for collection.
        
        Args:
            date_str: Date in YYYY-MM-DD format
            
        Returns:
            True if date is valid for collection, False otherwise
        """
        try:
            ...
            if target_date > self.today:
                logger.error(f"ERROR: Cannot collect future date {date_str} (today: {self.today_str})")
                return False
                
            logger.debug(f"Date {date_str} validated for collection")
            return True
    ...
    def _collect_with_progress(self,
        ...
        if not self.validate_collection_date(start_date):
            raise ValueError(f"Invalid start date for collection: {start_date}")
        
        if not self.validate_collection_date(end_date):
            raise ValueError(f"Invalid end date for collection: {end_date}")
        
        # Additional validation: ensure end date is not in the future
        if end > self.today:
            logger.warning(f"End date {end_date} is in the future, adjusting to today ({self.today_str})")
            end = self.today
        ...
        try:
            ...
            while current_date <= end:
                ...
                if not self.validate_collection_date(date_str):
                    logger.error(f"Skipping invalid date {date_str}")
